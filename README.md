...
智能体通过「工具调用（function calling）」完成操作，当前共 **15 个工具**：

| 功能 | 工具 | 说明 |
|------|------|------|
| 过滤 / 筛选 | `filter_rows` | 按条件过滤行（数值比较、包含、前缀、为空等，等级列支持"B及以上"这类等级比较） |
...
| 分组比例插行 | `insert_group_ratio` | 按分组统计各列等级/数值达标比例，并作为新行插入每个分组末尾 |
| 自定义计算 / 插行 | `run_python` | 执行 pandas 代码做任意复杂操作（如分组统计后插入汇总行），可用 `df`/`pd`/`grade_num` |
